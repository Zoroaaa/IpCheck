// ============================================================
//  checkProxyIP.js — ProxyIP（CF反代IP）可用性检测
// 
//  配置说明：
//  - TLS_SNI_HOST: 可通过环境变量 TLS_SNI_HOST 配置 TLS 握手目标域名
//                  默认使用硬编码报文（目标域名：chatgpt.com）
//                  如需修改，请重新生成 TLS Client Hello 报文
// 
//  修复：添加配置常量，提取魔法数字
// ============================================================
import { connect } from 'cloudflare:sockets';

// ---------- 配置常量 ----------
const MAX_RETRY = 4;                    // 最大重试次数
const BASE_TIMEOUT = 1000;              // 基础超时时间（毫秒）
const RETRY_DELAY_BASE = 200;           // 重试延迟基数（毫秒）
const HTTP_TRACE_TIMEOUT = 5000;        // HTTP trace 请求超时（毫秒）

// ---------- TLS 握手报文（预生成，目标 SNI: chatgpt.com）----------
// 注意：如需更换目标域名，需重新生成此报文
function 构建TLS握手() {
  const hex =
    '';
  return new Uint8Array(hex.match(/.{1,2}/g).map(b => parseInt(b, 16)));
}

async function 带超时连接({ hostname, port }, timeout) {
  const sock = connect({ hostname, port });
  await Promise.race([
    sock.opened,
    new Promise((_, rej) => setTimeout(() => rej(new Error('连接超时')), timeout)),
  ]);
  return sock;
}

function 带超时读取(reader, timeout) {
  return new Promise(resolve => {
    const id = setTimeout(() => resolve({ done: true, value: null, 超时: true }), timeout);
    reader.read().then(r => { clearTimeout(id); resolve({ ...r, 超时: false }); });
  });
}

/** TLS 握手验证，使用配置常量控制重试次数和超时 */
async function 验证反代IP(ip, port) {
  let lastErr = null;
  const t0 = performance.now();
  for (let i = 0; i < MAX_RETRY; i++) {
    let sock = null, r = null;
    try {
      const timeout = BASE_TIMEOUT + i * 500;
      sock = await 带超时连接({ hostname: ip, port }, timeout);
      const w = sock.writable.getWriter();
      r = sock.readable.getReader();
      await w.write(构建TLS握手());
      const { value, 超时 } = await 带超时读取(r, timeout);
      if (超时) throw new Error(`第${i + 1}次读取超时`);
      if (!value || value.length === 0) throw new Error(`第${i + 1}次无响应数据`);
      if (value[0] === 0x16) {
        try { r.cancel(); sock.close(); } catch (_) {}
        return [true, `第${i + 1}次验证有效ProxyIP`, Math.round(performance.now() - t0)];
      }
      lastErr = `第${i + 1}次非TLS响应(0x${value[0].toString(16).padStart(2, '0')})`;
      throw new Error(lastErr);
    } catch (e) {
      lastErr = `第${i + 1}次失败: ${e.message}`;
      const fatal = ['Connection refused', '连接被拒绝', 'Network unreachable', 'Host unreachable'];
      if (fatal.some(f => e.message.toLowerCase().includes(f.toLowerCase()))) break;
    } finally {
      try { if (r) r.cancel(); } catch (_) {}
      try { if (sock) sock.close(); } catch (_) {}
      await new Promise(res => setTimeout(res, 100));
    }
    if (i < MAX_RETRY - 1) await new Promise(res => setTimeout(res, RETRY_DELAY_BASE + i * 300));
  }
  return [false, lastErr || '连接验证失败', -1];
}

/** 解析 proxyip 字符串，提取 ip 和端口 */
function parseProxyIP(raw) {
  let ip = raw.toLowerCase();
  let port = 443;
  if (ip.includes('.tp')) {
    const m = ip.match(/\.tp(\d+)\./);
    if (m) port = parseInt(m[1]);
  } else if (ip.includes('[') && ip.includes(']:')) {
    port = parseInt(ip.split(']:')[1]);
    ip = ip.split(']:')[0] + ']';
  } else if (ip.includes(':')) {
    port = parseInt(ip.split(':')[1]) || 443;
    ip = ip.split(':')[0];
  }
  return { ip, port };
}

/**
 * 检测 ProxyIP 可用性
 * 先发 HTTP GET 到 CF trace 端点（验证反代能力），再做 TLS 握手（验证 443 端口代理）
 */
export async function CheckProxyIP(proxyIPRaw, colo = 'CF') {
  const { ip, port } = parseProxyIP(proxyIPRaw);

  // Step1: 发送明文 HTTP 请求，期望收到 CF 400 Bad Request 响应
  const sock = connect({ hostname: ip, port });
  try {
    const req =
      'GET /cdn-cgi/trace HTTP/1.1\r\n' +
      'Host: speed.cloudflare.com\r\n' +
      'User-Agent: CheckProxyIP/cmliu\r\n' +
      'Connection: close\r\n\r\n';
    const w = sock.writable.getWriter();
    await w.write(new TextEncoder().encode(req));
    w.releaseLock();

    const reader = sock.readable.getReader();
    let buf = new Uint8Array(0);
    while (true) {
      const { value, done } = await Promise.race([
        reader.read(),
        new Promise(res => setTimeout(() => res({ done: true }), HTTP_TRACE_TIMEOUT)),
      ]);
      if (done) break;
      if (value) {
        const nb = new Uint8Array(buf.length + value.length);
        nb.set(buf); nb.set(value, buf.length);
        buf = nb;
        const txt = new TextDecoder().decode(buf);
        if (txt.includes('\r\n\r\n') &&
          (txt.includes('Connection: close') || txt.includes('content-length'))) break;
      }
    }
    reader.releaseLock();
    await sock.close();

    const txt = new TextDecoder().decode(buf);
    const ok = txt.includes('cloudflare') && txt.includes('CF-RAY') &&
      txt.includes('The plain HTTP request was sent to HTTPS port') &&
      txt.includes('400 Bad Request') && buf.length > 100;

    if (!ok) {
      return { success: false, proxyIP: ip, portRemote: port, colo, responseTime: -1, message: '无法通过ProxyIP访问Cloudflare', timestamp: new Date().toISOString() };
    }

    // Step2: TLS 握手验证
    const [tlsOk, msg, ms] = await 验证反代IP(ip, port);
    return { success: tlsOk, proxyIP: ip, portRemote: port, colo, responseTime: tlsOk ? ms : -1, message: msg, timestamp: new Date().toISOString() };

  } catch (e) {
    return { success: false, proxyIP: -1, portRemote: -1, colo, responseTime: -1, message: e.message || String(e), timestamp: new Date().toISOString() };
  }
}

/**
 * /check?proxyip=... 路由处理器（ProxyIP 模式）
 */
export async function handleCheckProxyIP(request, url) {
  const raw = url.searchParams.get('proxyip');
  if (!raw) return new Response('Missing proxyip parameter', { status: 400 });
  if (!raw.trim()) return new Response('Invalid proxyip parameter', { status: 400 });
  if (!raw.includes('.') && !(raw.includes('[') && raw.includes(']')))
    return new Response('Invalid proxyip format', { status: 400 });

  const colo = request.cf?.colo || 'CF';
  const result = await CheckProxyIP(raw.trim(), colo);
  return new Response(JSON.stringify(result, null, 2), {
    status: result.success ? 200 : 502,
    headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
  });
}
